...
function Flags ()
{
    ...
    this.levelLoaded = false; //is the level text loaded?
...
function Images ()
{
    ...
    this.background.src = "../assets/desktopBackground.png";
    this.background.addEventListener ("load", function () {flags.backgroundLoaded = true;}); //sets flag when background is loaded.

    this.level = new Image ();
    this.level.src = "../assets/level.png";
    this.level.addEventListener ("load", function () {flags.levelLoaded = true;}); //sets flag when level image is loaded

    this.rotor = new Image (); //rotor image
    this.rotor.src = "../assets/rotor.png";
    this.rotor.addEventListener ("load", function () {flags.rotorLoaded = true;}); //sets flag when rotor is laoded.
}
...
function draw ()
{
    ...
    if (flags.backgroundLoaded)
    {
        ...
        ctx.drawImage (
            images.background,
            0, 0,
            images.background.width, images.background.height,
            0, 0,
            width, height
            );
    }
    ...
    //if level is loaded
    if (flags.levelLoaded)
    {
        //draw it
        ctx.drawImage (
            images.level,
            220, 90,
            images.level.width, images.level.height,
            0, 0,
            images.level.width, images.level.height
        );
    }
